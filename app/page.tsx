...
const generateDietTargetData = (allDates: string[]): MergedEntry[] => {
    const { dietStartDate, dietEndDate, dietStartWeight, dietEndWeight } = TARGET_CONFIG;
    ...
    return allDates.map(dateStr => {
        ...
        const weight = dietStartWeight - ((dietStartWeight - dietEndWeight) * daysPassed) / totalDays;
        return {
            date: dateStr,
            weight: null,
...
export default function Home() {
    ...
    const dietTargetData = useMemo(() => generateDietTargetData(allDates), [allDates]);
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                    ...
                    <label className="flex flex-col gap-2">
                        <span className="text-sm font-medium text-gray-700">
                            몸무게: <strong className="text-blue-600">{weight.toFixed(1)} kg</strong>
                        </span>
                        <input
                            type="range"
                            min="75"
